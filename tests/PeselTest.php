...
class PeselTest extends PHPUnit_Framework_TestCase {
	...
	function testInvalidLength(){
		$pesel = new Pesel('800529001766');
		
		$this->assertEquals($pesel->isValid(), false);
		$this->assertEquals($pesel->getValidity(), Pesel::VALIDITY_INVALID_LENGTH);
	}
	
	function testInvalidCharacters(){
		$pesel = new Pesel('abcdefgijkl');
		
		$this->assertEquals($pesel->isValid(), false);
		$this->assertEquals($pesel->getValidity(), Pesel::VALIDITY_INVALID_NUMERICNESS);
	}
